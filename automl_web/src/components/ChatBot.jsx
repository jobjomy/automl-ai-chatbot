...
import { Upload, Cpu, User, CheckCircle, AlertCircle, Loader,
         FileText, Download, Send, RefreshCw, MessageSquare, Copy, Check } from 'lucide-react'
...
const STEPS = {
  UPLOAD: 'upload', TASK_TYPE: 'task_type', TARGET_COLUMN: 'target_column',
  PRIORITY: 'priority', EXTENDED_DOCS: 'extended_docs',
  RUNNING: 'running', DONE: 'done', FOLLOWUP: 'followup',
}

const GREETING = "Hi! I'm **AutoML Agent** — your AI-powered ML engineer. Let's build a machine learning model together.\n\nStart by **uploading your CSV dataset** below."
...
// ── CodeBlock — copy-button code renderer for ReactMarkdown ──────
function CodeBlock({ inline, className, children }) {
  ...
  if (inline) {
    return (
      <code className="bg-surface border border-border/50 text-cyan px-1.5 py-0.5 rounded text-xs font-mono">
        {code}
      </code>
    )
  }
  ...
  return (
    <div className="relative group my-3">
      ...
      <pre className="bg-[#16161e] border border-border border-t-0 rounded-b-xl p-4 overflow-x-auto m-0">
        <code className="text-[#a0dcff] text-xs font-mono leading-relaxed whitespace-pre">
          {code}
        </code>
      </pre>
    </div>
...
// ── Atoms ──────────────────────────────────────────────────────────
function TypingDots() {
  return <div className="flex gap-1 items-center px-1 py-1">
...
function UserBubble({ children }) {
  return (
    <div className="flex gap-3 justify-end animate-slide-up">
      <div className="max-w-[82%] rounded-2xl rounded-tr-sm bg-accent/20 border border-accent/30 px-4 py-3 text-sm text-slate-200">{children}</div>
    ...
    <div className="mt-3 rounded-xl overflow-hidden border border-border text-xs font-mono">
      <div className="overflow-x-auto">
        <table className="w-full">
          <thead><tr className="bg-surface/80">
            {vis.map(c => <th key={c} className="px-3 py-2 text-left text-accent-bright font-medium truncate max-w-[100px]">{c}</th>)}
            {columns.length > 5 && <th className="px-3 py-2 text-muted">+{columns.length - 5} more</th>}
          </tr></thead>
          <tbody>{preview.slice(0,3).map((row,i) => (
            <tr key={i} className="border-t border-border/50">
              {vis.map(c => <td key={c} className="px-3 py-1.5 text-subtle truncate max-w-[100px]">
                {row[c]==null||row[c]==='' ? <span className="text-muted italic">null</span> : String(row[c]).slice(0,15)}
              </td>)}
              {columns.length > 5 && <td className="px-3 py-1.5 text-muted">...</td>}
            </tr>
          ))}</tbody>
        </table>
      </div>
...
function AgentLog({ logs }) {
  return (
    <div className="mt-3 space-y-1.5 font-mono text-xs max-h-52 overflow-y-auto pr-1">
      {logs.map((log, i) => (
        <div key={i} className={`flex items-start gap-2 ${log.status==='done'?'text-emerald-400':log.status==='error'?'text-red-400':'text-accent-bright'}`}>
          {log.status==='done' ? <CheckCircle size={11} className="mt-0.5 flex-shrink-0" />
           : log.status==='error' ? <AlertCircle size={11} className="mt-0.5 flex-shrink-0" />
...
function ReportBubble({ report, config, onRerun, onSwitchTask, onChangeTarget, onReset }) {
  ...
  const handle = async (type) => {
    ...
    try {
      ...
      if (type === 'py')    downloadPy(report, `automl_pipeline_${ts}.py`)
      if (type === 'ipynb') downloadIpynb(report, report, `automl_pipeline_${ts}.ipynb`)
      ...
      <div className="flex-1 min-w-0 rounded-2xl rounded-tl-sm bg-panel border border-emerald-500/20 overflow-hidden">
        ...
        <div className="flex items-center justify-between px-4 py-3 border-b border-emerald-500/20 bg-emerald-500/5">
          <div className="flex items-center gap-2">
            ...
            <span className="text-emerald-400 text-sm font-semibold">AutoML Report Ready</span>
          </div>
          <div className="flex items-center gap-2">
        ...
        {config && (
          <div className="flex flex-wrap gap-1.5 px-4 py-2.5 border-b border-border/50 bg-surface/20">
            {[
              ...
              <span key={item.k} className="text-xs px-2 py-0.5 rounded-md bg-surface border border-border">
                <span className="text-muted">{item.k}: </span>
                <span className="text-white font-medium capitalize">{item.v}</span>
              </span>
            ))}
          </div>
        )}
        ...
        {/* Download buttons */}
        <div className="flex items-center gap-2 px-4 py-2.5 border-b border-border/50 bg-surface/30 flex-wrap">
          ...
          {[
            ...
            { type:'py',    label:'.py',   cls:'text-blue-400 border-blue-400/30 hover:bg-blue-400/10' },
            { type:'ipynb', label:'.ipynb',cls:'text-orange-400 border-orange-400/30 hover:bg-orange-400/10' },
          ].map(b => (
            <button key={b.type} onClick={() => handle(b.type)} disabled={dl!==null}
              ...
              {dl===b.type ? <Loader size={10} className="animate-spin" /> : <Download size={10} />}
              {b.label}
            </button>
          ))}
          ...
          <div className="ml-auto flex items-center gap-1.5 flex-wrap">
            ...
            {['speed','accuracy','cost'].filter(p => p !== config?.priority).map(p => (
              <button key={p} onClick={() => onRerun({ priority: p })}
        ...
        <div className="flex items-center gap-2 px-4 py-2 border-b border-border/50 bg-surface/20 flex-wrap">
          ...
          {[{type:'classification',icon:'📊'},{type:'regression',icon:'📈'},{type:'clustering',icon:'🔵'}]
            .filter(t => t.type !== config?.taskType).map(t => (
              <button key={t.type} onClick={() => onSwitchTask(t.type)}
          ...
          <span className="ml-auto text-xs text-muted italic">same dataset · new task</span>
        </div>

        {/* Report content */}
        <div className={`px-4 py-4 overflow-y-auto transition-all ${expanded ? 'max-h-[700px]' : 'max-h-[420px]'}`}>
          <ReactMarkdown
...
const PillRow = ({ items, onSelect }) => (
  <div className="flex gap-3 pl-11 flex-wrap">
    {items.map(([label, value]) => (
      <button key={String(value)} onClick={() => onSelect(value, label)}
...
const ColumnPicker = ({ columns, onSelect }) => (
  <div className="pl-11 grid grid-cols-2 gap-1.5 max-h-52 overflow-y-auto pr-1">
    {columns.map((col, i) => (
      <button key={i} onClick={() => onSelect(i, col)}
        ...
        <span className="text-muted mr-1.5">{i}.</span>{col}
      </button>
    ))}
  </div>
)
...
// ── Follow-up input bar ────────────────────────────────────────────
function FollowUpBar({ onSend, onRerunWithParams, uploadData, disabled, currentTaskType, currentPriority }) {
  ...
  return (
    <div className="border-t border-border flex-shrink-0">
      {/* Suggestions */}
      <div className="pt-2">
        <FollowUpSuggestions onSelect={t => { setText(t); }} currentTaskType={currentTaskType} currentPriority={currentPriority} />
      </div>
      {/* Input */}
      <div className="flex items-center gap-2 p-3 pt-1">
        ...
        <button onClick={send} disabled={!text.trim() || disabled}
          ...
          {disabled ? <Loader size={15} className="animate-spin" /> : <Send size={15} />}
        </button>
      </div>
    </div>
  )
...
export default function ChatBot({ userId, initialSession }) {
  ...
  const [uploadData, setUploadData] = useState(() => initialSession?.uploadData || null)
  const [config, setConfig]         = useState(() =>
    initialSession?.config || { filePath:null, taskType:null, targetColumn:null, priority:null, extendedDocs:false }
  )
  ...
  useEffect(() => { bottomRef.current?.scrollIntoView({ behavior:'smooth' }) }, [messages, isTyping, logs])
  ...
  const handleFile = async (file) => {
    if (!file?.name.endsWith('.csv')) { addMsg('ai','⚠ Please upload a **.csv** file.'); return }
    ...
    try {
      ...
      addMsg('ai', `**Available columns (${data.columns.length}):**\n${data.columns.map((c,i)=>`\`${i}. ${c}\``).join('  ')}\n\nWhat type of ML problem is this?`)
  ...
  const handleExtendedDocs = async (value, label) => {
    ...
    setConfig(cfg)
    if (userId) { session.title = `${cfg.taskType} · ${cfg.targetColumn||'clustering'} · ${cfg.priority}`; session.config = cfg; session.status = 'running'; saveSession(userId, session) }
    await typeMsg(
      `**Configuration:**\n- Task: \`${cfg.taskType}\`  · Target: \`${cfg.targetColumn||'N/A'}\`\n- Priority: \`${cfg.priority}\`  · Extended Docs: \`${value?'Yes':'No'}\`\n\nLaunching **15-agent pipeline** 🚀`
    )
    addMsg('ai', '**Pipeline running** — agents working in real time:', { showLogs: true })
    setLogs([])
  ...
  const launchPipeline = async (cfg) => {
    try {
      const { job_id } = await runPipeline({
        filePath:cfg.filePath, taskType:cfg.taskType,
        targetColumn:cfg.targetColumn, priority:cfg.priority, extendedDocs:cfg.extendedDocs,
      })
      streamLogs(
        job_id,
        (log) => setLogs(l => [...l, log]),
          ...
          if (status === 'done') {
            ...
            const poll = async () => {
              ...
              if (data.report) {
                ...
                setStep(STEPS.DONE)
                addMsg('ai', '__REPORT__', { isReport: true, reportConfig: cfg })
                // Follow-up invite
                setTimeout(() => {
                  setMessages(m => [...m, {
                    role:'ai',
                ...
                if (userId) { session.status = 'done'; session.report = data.report; saveSession(userId, session) }
              } else if (attempts++ < 20) {
            ...
            addMsg('ai','⚠ Pipeline error. Check the uvicorn terminal and try again.')
            setStep(STEPS.FOLLOWUP)
          }
        }
      )
  ...
  const handleRerun = async (overrides) => {
    ...
    if (overrides.targetColumn) parts.push(`target → **${overrides.targetColumn}**`)
    if (overrides.taskType)     parts.push(`task → **${overrides.taskType}**`)
    addMsg('user', `🔄 Rerun with ${parts.join(', ')}`)
    await typeMsg(`Got it! Relaunching the pipeline with **${parts.join(', ')}**...\n\nSame dataset, new configuration. 🚀`)
    addMsg('ai', '**Pipeline running** — agents working in real time:', { showLogs: true })
    setStep(STEPS.RUNNING)
    if (userId) { session.status = 'running'; saveSession(userId, session) }
    await launchPipeline(cfg)
  }
  ...
  const handleChangeTarget = async () => {
    addMsg('ai',
      'Pick a new **target column** from your dataset (' + (uploadData ? uploadData.columns.length : 0) + ' columns available):',
  ...
  const handleSwitchTask = async (newTaskType) => {
    ...
    if (isCluster) {
      ...
      const cfg = { ...configRef.current, taskType: newTaskType, targetColumn: null }
      setConfig(cfg)
      await typeMsg(
...
Which column should be the **target variable**?`
  ...
  const handleFollowUp = async (text) => {
    ...
    const targetMatch   = uploadData?.columns.find(c => lower.includes(c.toLowerCase()))
    const isRerun = lower.includes('rerun') || lower.includes('run again') || lower.includes('try again') || lower.includes('different')

    // Task type switch detection
    if (lower.includes('switch') || lower.includes('change') || lower.includes('try')) {
      if (lower.includes('regression'))     { await handleSwitchTask('regression'); return }
      if (lower.includes('clustering'))     { await handleSwitchTask('clustering'); return }
      if (lower.includes('classification')) { await handleSwitchTask('classification'); return }
    }

    // Target column change detection
    if (lower.includes('target') || lower.includes('different column') || lower.includes('change column') || lower.includes('another column')) {
      await handleChangeTarget(); return
    }
...
Answer the user's question concisely and helpfully based on their specific report and data. Be direct and specific.`
      ...
      const answer = data.content?.map(b => b.text || '').join('') || 'Sorry, I could not get a response.'
      setIsTyping(false)
      addMsg('ai', answer)
    } catch(e) {
      setIsTyping(false)
      // Fallback: handle locally
      addMsg('ai', handleFollowUpLocally(text, configRef.current, reportRef.current))
    }
  ...
  const reset = () => {
    ...
    setIsTyping(false); setUploadData(null); setLogs([]); setReport(null); setFollowupBusy(false)
    setConfig({ filePath:null, taskType:null, targetColumn:null, priority:null, extendedDocs:false })
  }

  // ── Change target column after report ────────────────────────
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 overflow-y-auto p-4 md:p-5 space-y-4">
            ...
            {msg.role === 'ai'
              ? msg.isReport
                ? <ReportBubble
                    ...
                    onReset={reset} />
                : <AIBubble>
                    <ReactMarkdown className="prose prose-invert prose-sm max-w-none prose-p:my-0.5 prose-code:bg-surface prose-code:px-1 prose-code:rounded prose-code:text-accent-bright" components={MD_COMPONENTS}>
                      {msg.content}
                    </ReactMarkdown>
                    ...
                    {msg.showTargetPicker && uploadData && (
                      <div className="mt-3 grid grid-cols-2 gap-1.5 max-h-40 overflow-y-auto">
                        {uploadData.columns.map((col, i) => (
                          <button key={i} onClick={() => handleRerun({ targetColumn: col })}
        ...
        {/* Step pickers */}
        {step===STEPS.TASK_TYPE     && !isTyping && <PillRow items={[['📊 Classification','classification'],['📈 Regression','regression'],['🔵 Clustering','clustering']]} onSelect={handleTaskType} />}
        {step===STEPS.TARGET_COLUMN && !isTyping && uploadData && <ColumnPicker columns={uploadData.columns} onSelect={handleTargetColumn} />}
        {step===STEPS.PRIORITY      && !isTyping && <PillRow items={[['⚡ Speed','speed'],['🎯 Accuracy','accuracy'],['💰 Cost','cost']]} onSelect={handlePriority} />}
        {step===STEPS.EXTENDED_DOCS && !isTyping && <PillRow items={[['✅ Yes, include it',true],['❌ No thanks',false]]} onSelect={handleExtendedDocs} />}
      ...
      {step === STEPS.UPLOAD && (
        <div className="p-4 border-t border-border flex-shrink-0">
          <div onDragOver={e=>{e.preventDefault();setDragging(true)}} onDragLeave={()=>setDragging(false)}
            ...
            <p className="text-sm text-subtle">Drop your CSV here, or <span className="text-accent-bright">click to browse</span></p>
      ...
      {step === STEPS.RUNNING && (
        <div className="p-4 border-t border-border flex-shrink-0">
          <div className="flex items-center gap-3 px-4 py-3 rounded-xl bg-surface border border-border text-sm text-muted">
            <Loader size={14} className="animate-spin text-accent-bright flex-shrink-0" />
